...
function getApprovedArchitecture(project: Project): ArchitectureDraft | undefined {
  const latest = getLatestArtifact(getProjectArtifacts(project), ARTIFACT_TYPES.ARCHITECTURE_DRAFT);

  if (!latest || latest.status !== 'approved') {
    return undefined;
  }

  return parseArtifactContent<ArchitectureDraft>(latest.content);
}
