...
/**
 * The QA Draft asks for 28 free-text and list fields (QA overview through
 * recommended next steps) — comparable in size to the Backend and Frontend
 * Drafts, so it keeps the same larger output budget for the same
 * truncation reason. app/routes/api.generate-text.ts fails gracefully (a
 * clear 400 message surfaced via `errorMessage`) if the selected model
 * can't support this many output tokens.
 */
const MAX_OUTPUT_TOKENS = 8192;

/**
 * Sprint 21 — the QA Draft feature, built on the exact Sprint 20
 * FrontendDraftPanel pattern: a "Generate QA Draft" button, a confirm step,
 * a loading state, a structured preview with Approve/Discard/Regenerate,
 * and a persistent status line once a draft exists. Approve here only ever
 * changes this artifact's own `status` — it never generates test code
 * (Jest, Vitest, Cypress, Playwright, Selenium, Puppeteer), connects to
 ...
 * Backend Draft, or the Frontend Draft. Generation is disabled until the
 * Frontend Draft has been approved (`qaEngineerEngine.canGenerateQA`). All
 * context gathering/prompt building/parsing goes through `qaEngineerEngine`;
 ...
 */
export function QaDraftPanel({ project }: QaDraftPanelProps) {
  const {
    phase,
    setPhase,
    errorMessage,
    ...
    handleApprove,
    handleDiscard,
  } = useDraftPanel<QADraft, ReturnType<typeof qaEngineerEngine.buildQAContext>>({
    project,
    artifactType: ARTIFACT_TYPE,
    titlePrefix: 'QA Draft',
    ...
    return (
      <div className="flex flex-wrap items-center gap-3">
        <button
          type="button"
          disabled
          title="Approve the Frontend Draft first."
        ...
        >
          ...
          <span className="text-sm font-medium">Generate QA Draft</span>
        </button>
        <span className="text-xs text-bolt-elements-textTertiary">Approve the Frontend Draft first.</span>
      </div>
    );
  }
    ...
    <div className="space-y-3">
      {isPreviewing ? (
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-lg border border-dashed border-bolt-elements-borderColor/60 p-3">
          <span className="text-xs text-bolt-elements-textTertiary">
            The AI QA Engineer will draft a complete testing strategy from your approved frontend design and current
        ...
        <div className="flex items-center gap-2 text-xs text-bolt-elements-textTertiary px-1">
          ...
          The AI QA Engineer is drafting the testing strategy…
        </div>
      ) : (
        <div className="flex flex-wrap items-center gap-3">
          <button
      ...
      {latest && !isPreviewing && statusMeta && (
        <div className="flex flex-wrap items-center gap-2 text-[11px] text-bolt-elements-textTertiary px-1">
          <span className="font-medium text-bolt-elements-textSecondary">QA Draft</span>
          <span
            className={classNames(
