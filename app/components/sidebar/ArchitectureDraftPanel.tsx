 ...
 * and persisting the result — the state machine itself lives in
 * app/lib/hooks/useDraftPanel.ts (Sprint 16), shared with every other
 * status-only-approval draft panel.
 */
export function ArchitectureDraftPanel({ project }: ArchitectureDraftPanelProps) {
  const {
    ...
    handleApprove,
    handleDiscard,
  } = useDraftPanel<ArchitectureDraft, ReturnType<typeof solutionArchitectEngine.buildArchitectureContext>>({
    project,
    artifactType: ARTIFACT_TYPE,
    titlePrefix: 'Architecture Draft',
