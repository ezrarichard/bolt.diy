...
import { devopsEngineerEngine } from '~/lib/projects/devopsEngineerEngine';
...
interface DevOpsDraftPanelProps {
  project: Project;
}
...
/**
 * The DevOps Draft asks for 31 free-text and list fields (DevOps overview
 * through recommended next steps) — the largest draft shape yet, so it
 * keeps the same larger output budget the Backend, Frontend, and QA Drafts
...
/**
 * Sprint 22 — the DevOps Draft feature, built on the exact Sprint 21
 * QaDraftPanel pattern: a "Generate DevOps Draft" button, a confirm step, a
 * loading state, a structured preview with Approve/Discard/Regenerate, and
 * a persistent status line once a draft exists. Approve here only ever
 * changes this artifact's own `status` — it never generates a Dockerfile,
 * GitHub Actions workflow, Kubernetes manifest, Terraform configuration, or
 * shell script, never deploys or provisions anything, and never mutates
 * Project Knowledge, the Architecture Draft, the Database Design Draft, the
 * UI/UX Draft, the Backend Draft, the Frontend Draft, or the QA Draft.
 * Generation is disabled until the QA Draft has been approved
 * (`devopsEngineerEngine.canGenerateDevOps`). All context
 * gathering/prompt building/parsing goes through `devopsEngineerEngine`;
 ...
 */
export function DevOpsDraftPanel({ project }: DevOpsDraftPanelProps) {
  const {
    phase,
    setPhase,
    errorMessage,
    ...
    handleApprove,
    handleDiscard,
  } = useDraftPanel<DevOpsDraft, ReturnType<typeof devopsEngineerEngine.buildDevOpsContext>>({
    project,
    artifactType: ARTIFACT_TYPE,
    titlePrefix: 'DevOps Draft',
    ...
    return (
      <div className="flex flex-wrap items-center gap-3">
        <button
          type="button"
          disabled
          title="Approve the QA Draft first."
        ...
        >
          ...
          <span className="text-sm font-medium">Generate DevOps Draft</span>
        </button>
        <span className="text-xs text-bolt-elements-textTertiary">Approve the QA Draft first.</span>
      </div>
    );
  }
    ...
    <div className="space-y-3">
      {isPreviewing ? (
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-lg border border-dashed border-bolt-elements-borderColor/60 p-3">
          <span className="text-xs text-bolt-elements-textTertiary">
            The AI DevOps Engineer will draft an operational strategy from your approved QA strategy and current project
        ...
        <div className="flex items-center gap-2 text-xs text-bolt-elements-textTertiary px-1">
          ...
          The AI DevOps Engineer is drafting the operational strategy…
        </div>
      ) : (
        <div className="flex flex-wrap items-center gap-3">
          <button
      ...
      {latest && !isPreviewing && statusMeta && (
        <div className="flex flex-wrap items-center gap-2 text-[11px] text-bolt-elements-textTertiary px-1">
          <span className="font-medium text-bolt-elements-textSecondary">DevOps Draft</span>
          <span
            className={classNames(
