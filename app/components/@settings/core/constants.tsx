 ...
 * `className` it is given (defaulting to the previous `w-4 h-4`) so a caller can size them like any
 * lucide icon — before this they hardcoded their size and silently ignored the prop, which left them
 * rendering 16px next to 18px lucide icons in the Control Panel's card grid.
 */
interface BrandIconProps {
  className?: string;
}

const GitLabIcon = ({ className = 'w-4 h-4' }: BrandIconProps) => (
...
const VercelIcon = ({ className = 'w-4 h-4' }: BrandIconProps) => (
  <svg viewBox="0 0 24 24" className={className}>
    <path fill="currentColor" d="M12 2L2 19.777h20L12 2z" />
  </svg>
);

const NetlifyIcon = ({ className = 'w-4 h-4' }: BrandIconProps) => (
  <svg viewBox="0 0 24 24" className={className}>
    <path
      fill="currentColor"
...
const SupabaseIcon = ({ className = 'w-4 h-4' }: BrandIconProps) => (
  <svg viewBox="0 0 24 24" className={className}>
    <path
      fill="currentColor"
